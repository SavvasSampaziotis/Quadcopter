 ...
 */
struct chip_cfg_s {
    /* Matches gyro_cfg >> 3 & 0x03 [FSR = Full Scale Range] */
    unsigned char gyro_fsr;
    /* Matches accel_cfg >> 3 & 0x03 [FSR = Full Scale Range] */
    unsigned char accel_fsr;
...
int mpu_set_sensors(unsigned char sensors)
{
	...
	/*Serial.print("@user_ctl was:");
	Serial.print(user_ctrl);
	Serial.print("|");*/
    if (sensors & INV_XYZ_COMPASS) {
        data = AKM_SINGLE_MEASUREMENT;
        user_ctrl |= BIT_AUX_IF_EN;
	/*	Serial.print("@user_ctl is:");
		Serial.print(user_ctrl);
		Serial.print("|");*/
    } else {
        data = AKM_POWER_DOWN;
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
		...
		/*Serial.print("@ezbrzvz/");
		Serial.print(st.reg->user_ctrl);
		Serial.print("|");*/
        if (i2c_read(st.hw->addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        if (st.chip_cfg.sensors & INV_XYZ_COMPASS)
